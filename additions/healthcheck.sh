...
PEER=$(grep -i "^Endpoint" "/etc/amnezia/amneziawg/wg0.conf" | head -n1 | cut -d'=' -f2 | tr -d ' ')
...
PEER=$(grep -i "^AllowedIP" "/etc/amnezia/amneziawg/wg0.conf" | head -n1 | cut -d'=' -f2 | tr -d ' ' | awk -F',' '{print $1}' | awk -F'/' '{print $1}')
...
if [ $? -eq 0 ]; then
    ...
    if [ "$FAILURE_COUNT" -ge 6 ]; then
        awg-quick down wg0 && awg-quick up wg0 > /dev/null 2>&1
        echo 0 > "$HEALTHCHECK_FILE"
    fi
    exit 1
fi